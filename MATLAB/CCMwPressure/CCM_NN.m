function [duplex_final,E_final,duplex_t,E_t] = CCM_NN(size_ratio,NN_array,NP_array,d_array,rho_AA,rho_BB,sigma,linker_AT,linker_BT,NP_exp,comp,comp_m)
% Input
...
for i = 1:length(d_array)
    d = d_array(i);
    [overlap{i},area_total] = area_overlap(dna_increase,d,comp,comp_m);
end
...
for i = 1:length(d_array)
    ...
    % Restriction from stoichiometric
    % Particle A
    restrict_stoich_A = NP_Aexp/NP_A;
    % Particle B
    restrict_stoich_B = NP_Bexp/NP_B;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Repulsion Energy
    E_A_r = repulsive_Ac*restrict_stoich_A*restrict_stoich_B + (repulsive_AAs + repulsive_ABs)*restrict_stoich_A*restrict_stoich_A;
    E_B_r = repulsive_Bc*restrict_stoich_B*restrict_stoich_A + (repulsive_BBs + repulsive_BAs)*restrict_stoich_B*restrict_stoich_B;
    ...
    duplex_AAc = (area_overlap_AB*NN_A_B/area_total_A)*restrict_area_A*restrict_linker_AAc*restrict_stoich_A*restrict_stoich_B;
    % Percent linker B cross hybridized
    duplex_ABc = (area_overlap_AB*NN_A_B/area_total_A)*restrict_area_A*restrict_linker_ABc*restrict_stoich_A*restrict_stoich_B;
    
    % Cross hybridization for particle B
    % Percent linker B cross hybridized
    duplex_BBc = (area_overlap_BA*NN_B_A/area_total_B)*restrict_area_B*restrict_linker_BBc*restrict_stoich_A*restrict_stoich_B;
    % Percent linker A cross hybridized
    duplex_BAc = (area_overlap_BA*NN_B_A/area_total_B)*restrict_area_B*restrict_linker_BAc*restrict_stoich_A*restrict_stoich_B;
    
    % Self hybridization for particle A
    % Percent linker A self hybridized
    duplex_AAs = (area_overlap_AA*NN_A_A/area_total_A)*restrict_area_A*restrict_linker_AAs*restrict_stoich_A*restrict_stoich_A;
    % Percent linker B self hybridized
    duplex_ABs = (area_overlap_AA*NN_A_A/area_total_A)*restrict_area_A*restrict_linker_ABs*restrict_stoich_A*restrict_stoich_A;
    
    % Self hybridization for particle B
    % Percent linker B self hybridized
    duplex_BBs = (area_overlap_BB*NN_B_B/area_total_B)*restrict_area_B*restrict_linker_BBs*restrict_stoich_B*restrict_stoich_B;
    % Percent linker A self hybridized
    duplex_BAs = (area_overlap_BB*NN_B_B/area_total_B)*restrict_area_B*restrict_linker_BAs*restrict_stoich_B*restrict_stoich_B;
    
    % Total duplex
    duplex_A = (duplex_AAc + duplex_AAs + duplex_ABc + duplex_ABs);
    duplex_B = (duplex_BBc + duplex_BBs + duplex_BAc + duplex_BAs);
    
    % Repulsion from Free linkers
    if restrict_stoich_A > 1.0
        E_rA_stoich = linker_AT*NP_A*(restrict_stoich_A - 1);
    else
        E_rA_stoich = 0;
    end
    
    if restrict_stoich_B > 1.0
        E_rB_stoich = linker_BT*NP_B*(restrict_stoich_B - 1);
    else
        E_rB_stoich = 0;
    end
    
    E_r = (E_A_r + E_B_r) + 1.0*(E_rA_stoich + E_rB_stoich);
